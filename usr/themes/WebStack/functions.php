...
<hr>
   ...
   <p class="previous-tab"><strong><svg  class="icon zmki_aliico" aria-hidden="true"><use xlink:href="#icon-chicken"></use></svg> 主题版本号: </strong>2.5.06 <span></span></p>
<hr>
...
<input type="submit" name="type" class="btn btn-s" value="备份模板设置数据" />&nbsp;&nbsp;<input type="submit" name="type" class="btn btn-s" value="还原模板设置数据" />&nbsp;&nbsp;<input type="submit" name="type" class="btn btn-s" value="删除备份数据" /></form><br>';
 ...
 <p>ZMKI交流群:<a href="https://www.zmki.cn/doc/33773.html" target="_blank">查看用户群</a></p>
...
<?php
    ...
    // favicon
    $favicon = new Typecho_Widget_Helper_Form_Element_Text('favicon', NULL, '/usr/themes/WebStack/images/20191218-3d077e8df0bf0.png', _t('<svg  class="icon zmki_aliico" aria-hidden="true"><use xlink:href="#icon-edit"></use></svg> Favicon'), _t('请输入favicon站标的Url(ico Png jpg)等图片格式均可，刷新浏览器缓存后生效'));
    ...
    // 开启内页
    $isLink = new Typecho_Widget_Helper_Form_Element_Radio('isLink', array('0' => _t('禁用'), '1' => _t('启用')), '1', _t('开启内页'), _t("是否启用直接跳转，默认开启"));
...
为空，则程序自动获取手动填写的Icon图标。此功能依赖于API接口<br>默认：https://api.zmki.cn/i/ge.php?url= <br>更多查看 <a href="https://www.zmki.cn/doc/webstack-doc">导航帮助</a> <br>注意：此功能加载速度受限于api速度，如站点内容较大，可自建icon图片接口 <a href="https://www.zmki.cn/doc/33771.html">自建icon图片接口教程</a>'));
